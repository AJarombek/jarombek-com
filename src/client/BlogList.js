...
class BlogList extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!this.state.posts) {
      ...
      this.fetchPostsAndUpdate(url, postPage, queryStr, query).catch(() => {
        this.setState({
          shouldUpdate: true,
          posts: []
        });
  ...
  async fetchPostsAndUpdate(url = '/api/post/preview', pageNumber = 1, query = '', defaultSearch = '') {
    const { posts, first, prev, next, last } = await BlogDelegator.fetchPosts(this.baseUrl, url);
  ...
  queryPosts(query) {
    ...
    this.fetchPostsAndUpdate(`/api/post/preview?query="${query}"`, 1, query, query).catch(() => {
      this.setState({
        shouldUpdate: true,
        posts: []
      });
