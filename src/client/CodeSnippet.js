...
class CodeSnippet extends React.Component {
  ...
  languageInfo(language) {
    const matchedLanguage = this.languages.filter((lang) => lang.prop === language || lang.title === language)[0];
  ...
  render() {
    ...
    return (
      <figure className="jarombek-code-snippet">
        <code title={title} ref={(code) => (this.highlightedCode = code)} className={`${highlightClass} hljs`}>
          {children}
        </code>
      </figure>
    );
