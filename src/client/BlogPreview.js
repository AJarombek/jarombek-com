...
const BlogPreview = ({ name, title, type, date, tags = [], preview }) => {
  ...
  if (extraTagCount > 0) {
    tags = [...tags.filter((tag, index) => index < 2), { name: `+${extraTagCount} More` }];
  }
  ...
  return (
    <div className="jarombek-blog-preview">
      <div className="jarombek-blog-preview-headers">
        ...
        <p className="jarombek-blog-preview-date">{moment(date).format('MMMM Do, YYYY')}</p>
      </div>
      ...
      <div className="jarombek-blog-preview-content">{preview}</div>
      <Link className="jarombek-blog-preview-footer" to={`/blog/${name}`}>
        <Button activeColor="secondary" passiveColor="transparent" borderColor="none" size="small" font="longway">
          READ MORE
        </Button>
      </Link>
    </div>
